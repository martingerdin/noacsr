...
#' environment variables DB_USERNAME and DB_PASSWORD. `noacsr` uses
#' the package `dotenv` to access environment variables and load any
#' .env file in the project directory. We therefore recommed storing
#' the username and password in an .env file in the project directory,
#' but make sure this file is ignored by git so that you don't put it
#' in version control by mistake. If you created your project with
#' `noacsr::create()` then this has been taken care of.
...
#' @param db.name Character. The name of the database. No default.
...
import_data <- function(user = NULL,
    ...
    for (argument in list(db.name, table.names))
        if (is.null(argument)) pretty_stop("You have to provide both database and table name")
    ...
    if (is.null(user)) {
        ...
        if (user == "" & interactive() & ask) {
            user <- askpass::askpass(prettify_text("We did not find your username for accessing the database in the environment variable called DB_USERNAME. Please enter it here:"))
            pretty_message ("Do you want to save your username in a local .env file for future access? Please remember to add this .env file to .gitignore. (Y/N)")
    ...
    if (is.null(password)) {
        ...
        if (password == "" & interactive() & ask) {
            password <- askpass::askpass(prettify_text("We did not find your password for accessing the database in the environment variable called DB_PASSWORD. Please enter it here:"))
            pretty_message ("Do you want to save your password in a local .env file for future access? Please remember to add this .env file to .gitignore.")
            answer <- readLines(n = 1)
            if (tolower(answer) == "y")
